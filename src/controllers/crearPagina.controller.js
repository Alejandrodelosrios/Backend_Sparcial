...
class CrearPaginaController {
  // Propiedades para almacenar contexto durante la generación
  allElements = [];
  allConnections = [];
  ...
  exportarSpringBootDesdeSala = async (req, res) => {
    ...
    try {
      ...
      console.log(`🏛️  Clases encontradas: ${classElements.length}`);
      ...
      // ✅ EXTRAER CONNECTIONS (Relaciones) del XML
      let connections = [];
      ...
      if (connections.length > 0) {
        connections.forEach((conn, i) => {
          const sourceId = conn.source || conn.sourceId;
          const targetId = conn.target || conn.targetId;
          console.log(
            `    ${i + 1}. ${conn.type}: ${sourceId} (${conn.sourceMultiplicity}) -> ${targetId} (${conn.targetMultiplicity})`
          );
        });
      } else {
        console.log(`    ⚠️  No se encontraron relaciones en el diagrama`);
      }
...
public class DemoApplication {
  ...
  generarEndpointsRelaciones = (className, relacionesElement) => {
    ...
    relacionesElement.forEach((relacion) => {
      // ✅ Remover sufijos ID/Id/id de forma case-insensitive
      const nombreRelacion = relacion.fkName.replace(/ID$/i, "");
...
`;
      ...
      // ✅ SOLUCIÓN: Agregar @JsonProperty para mantener nombres exactos en JSON
      annotations += `    @JsonProperty("${exactColumnName}")
`;
    }
...
`;
  ...
  procesarElementosConRelaciones = (elements, connections) => {
    ...
    // Agregar foreign keys a los elementos
    console.log(`\n🔑 Agregando foreign keys a las entidades...`);
    processedElements.forEach((element) => {
      if (relacionesMap[element.id] && relacionesMap[element.id].length > 0) {
        console.log(`   ${element.name}: ${relacionesMap[element.id].length} FK(s)`);
    ...
    console.log("\n✅ Procesamiento completo con relaciones terminado");
    console.log(`📊 Resumen:`);
    console.log(`   - Elementos procesados: ${processedElements.length}`);
    console.log(`   - Relaciones identificadas: ${Object.keys(relacionesMap).filter(k => relacionesMap[k].length > 0).length}`);
  ...
  crearProyectoSpringBootCompleto = async (projectName, processedData) => {
    ...
    // ✅ Guardar elementos y conexiones como propiedades de la clase
    // para que estén disponibles en identificarRelacionesEntrantes
    this.allElements = elements;
    this.allConnections = connections || [];
    
    console.log(`\n🏗️  Creando estructura de proyecto Spring Boot...`);
    console.log(`   Elementos totales: ${elements.length}`);
    console.log(`   Conexiones totales: ${this.allConnections.length}`);
  ...
  generarEntidadConRelaciones = async (
    ...
    // ✅ PASO 1: Identificar relaciones bidireccionales
    const relacionesEntrantes = this.identificarRelacionesEntrantes(
      element,
      this.allElements,
      this.allConnections
    );
    
    console.log(`\n📝 Generando entidad: ${className}`);
    console.log(`   Relaciones salientes (FK aquí): ${relacionesElement.length}`);
    console.log(`   Relaciones entrantes (FK en otra tabla): ${relacionesEntrantes.length}`);
    
    // ✅ PASO 2: Recopilar imports necesarios
    const importsNecesarios = new Set();
    
    // Imports para relaciones salientes
    relacionesElement.forEach(rel => {
      if (rel.tipoRelacion !== 'inheritance') {
        importsNecesarios.add(rel.referenciaA);
      }
    });
    
    // Imports para relaciones entrantes
    relacionesEntrantes.forEach(rel => {
      importsNecesarios.add(rel.claseOrigen);
    });
...
`;
    ...
    // ✅ PASO 3: Agregar imports de entidades relacionadas
    importsNecesarios.forEach(nombreClase => {
      if (nombreClase !== className) {
        entityContent += `import com.example.demo.entity.${nombreClase};\n`;
      }
    });
...
`;
    ...
    // ✅ PASO 6: Relaciones salientes (donde ESTA clase tiene la FK)
    // NO generar campo Long xxxId separado, solo el objeto con @ManyToOne/@OneToOne
    if (relacionesElement && relacionesElement.length > 0) {
      entityContent += `    // ===== RELACIONES (Esta entidad mantiene la FK) =====
`;
      for (const relacion of relacionesElement) {
        if (relacion.tipoRelacion !== "inheritance") {
          entityContent += this.generarRelacionConFK(relacion);
        }
      }
    }
    
    // ✅ PASO 7: Relaciones entrantes (donde OTRA clase tiene la FK)
    // Aquí generamos @OneToMany o el lado inverso de @OneToOne
    if (relacionesEntrantes && relacionesEntrantes.length > 0) {
      entityContent += `    // ===== RELACIONES INVERSAS (Otra entidad mantiene la FK) =====
`;
      relacionesEntrantes.forEach(rel => {
        entityContent += this.generarRelacionInversa(rel, className);
      });
    }
    
    // ✅ PASO 8: Resto de la entidad (constructores, getters, setters, etc.)
    entityContent += this.generarRestoEntidadCorregido(
      className,
      element,
      relacionesElement,
      relacionesEntrantes,
    ...
    console.log(`✅ Entidad ${className}.java generada correctamente`);
  };

  /**
   * Generar SOLO el objeto relacionado con @ManyToOne/@OneToOne
   * SIN generar el campo Long xxxId por separado
   */
  generarRelacionConFK = (relacionInfo) => {
    // ✅ Remover sufijos ID/Id/id de forma case-insensitive
    const nombreCampo = relacionInfo.fkName
      .replace(/ID$/i, '')  // Remueve ID, Id, id al final (case-insensitive)
      .replace(/Id$/, '');   // Fallback por si acaso
    const tipoCampo = relacionInfo.referenciaA;
    const nombreFK = relacionInfo.fkName;
    
    let codigo = `    /**
     * Relación: ${relacionInfo.tipoRelacion} → ${relacionInfo.referenciaA}
     * FK: ${nombreFK} (mantenida automáticamente por JPA)
     */
`;
    ...
    switch (relacionInfo.tipoRelacionJPA) {
      case '@ManyToOne':
        codigo += `    @ManyToOne(fetch = ${fetchType}`;
        if (relacionInfo.esComposicion) {
          codigo += `, cascade = CascadeType.ALL`;
        }
        codigo += `)
    @JoinColumn(name = "${nombreFK}", nullable = false)
`;
        if (relacionInfo.esComposicion) {
          codigo += `    @JsonBackReference
`;
        }
        break;
        
      case '@OneToOne':
        codigo += `    @OneToOne(fetch = ${fetchType})
    @JoinColumn(name = "${nombreFK}", nullable = false)
`;
        break;
    }
    ...
    // ✅ SOLUCIÓN: Agregar @JsonProperty para mantener nombres exactos en JSON
    codigo += `    @JsonProperty("${nombreCampo}")
`;
    codigo += `    private ${tipoCampo} ${nombreCampo};
...
`;
  ...
  /**
   * Generar el lado inverso de una relación (sin FK física aquí)
   * Ejemplo: Usuario tiene List<Proyecto> proyectos
   */
  generarRelacionInversa = (relacionInfo, currentClassName) => {
    const nombreCampo = relacionInfo.claseOrigen.toLowerCase() + 's';
    const tipoColeccion = relacionInfo.claseOrigen;
    
    let codigo = `    /**
     * Relación inversa: ${relacionInfo.tipo}
     * FK mantenida en: ${relacionInfo.claseOrigen}
     */
`;
    
    switch (relacionInfo.tipo) {
      case 'OneToMany':
        codigo += `    @OneToMany(mappedBy = "${relacionInfo.mappedBy}"`;
        if (relacionInfo.esComposicion) {
          codigo += `, cascade = CascadeType.ALL, orphanRemoval = true`;
        }
        codigo += `)
`;
        if (relacionInfo.esComposicion) {
          codigo += `    @JsonManagedReference
`;
        } else {
          codigo += `    @JsonIgnore
`;
        }
        codigo += `    private List<${tipoColeccion}> ${nombreCampo} = new ArrayList<>();

`;
        break;
        
      case 'OneToOne':
        codigo += `    @OneToOne(mappedBy = "${relacionInfo.mappedBy}")
    @JsonIgnore
    private ${tipoColeccion} ${relacionInfo.claseOrigen.toLowerCase()};
...
public class ${className}Controller {
  ...
  analizarRelaciones = (elements, connections) => {
    ...
    connections.forEach((conn, index) => {
      console.log(`\n   📌 Procesando conexión #${index + 1}:`);
      
      const tipoRelacion = conn.type;
      // ✅ CORRECCIÓN: Soportar tanto source/target como sourceId/targetId
      const sourceId = conn.source || conn.sourceId;
      const targetId = conn.target || conn.targetId;
      const sourceMultiplicity = conn.sourceMultiplicity || '1';
      const targetMultiplicity = conn.targetMultiplicity || '1';
      
      console.log(`      Tipo: ${tipoRelacion}`);
      ...
      if (relacionInfo) {
        ...
        console.log(`      ⚠️  No se pudo determinar la relación`);
      }
    });
  ...
  /**
   * Identificar relaciones donde OTRA entidad tiene la FK hacia esta
   * Para generar el lado inverso (@OneToMany, etc.)
   */
  identificarRelacionesEntrantes = (element, allElements, connections) => {
    const relacionesEntrantes = [];
    
    if (!connections || !allElements) return relacionesEntrantes;
    ...
    connections.forEach(conn => {
      ...
      const normalizarMult = (mult) => {
        mult = mult.toString().trim();
        if (mult === '1..*' || mult === '0..*' || mult === 'n' || mult === 'N' || mult === '*') {
          return '*';
        }
        if (mult === '1..1') return '1';
        return mult;
      };
      
      const sourceMult = normalizarMult(sourceMultiplicity);
      const targetMult = normalizarMult(targetMultiplicity);
      
      // Verificar si esta entidad es el SOURCE de una relación 1:*
      // Usuario (1) --> Proyecto (*) = Usuario es SOURCE
      if (sourceId === element.id) {
        const targetElement = allElements.find(e => e.id === targetId);
        
        if (!targetElement) return;
        
        // 1:* → Esta entidad es el "1", la otra es "*" (tiene la FK)
        if (sourceMult === '1' && targetMult === '*') {
          relacionesEntrantes.push({
            tipo: 'OneToMany',
            claseOrigen: targetElement.name,
            claseOrigenId: targetId,
            mappedBy: element.name.toLowerCase(),
            tipoRelacion: conn.type,
            esComposicion: conn.type === 'composition'
          });
          console.log(`         ← ${targetElement.name} (muchos) apunta hacia ${element.name} (uno)`);
        }
        // 1:1 bidireccional
          ...
          console.log(`         ← ${targetElement.name} (uno) tiene FK hacia ${element.name} (uno)`);
        }
      }
      
      // Verificar si esta entidad es el TARGET de una relación *:1
      // Proyecto (*) --> Usuario (1) = Usuario es TARGET
      if (targetId === element.id) {
        const sourceElement = allElements.find(e => e.id === sourceId);
        ...
        if (sourceMult === '*' && targetMult === '1') {
          relacionesEntrantes.push({
            tipo: 'OneToMany',
            claseOrigen: sourceElement.name,
            claseOrigenId: sourceId,
            mappedBy: element.name.toLowerCase(),
            tipoRelacion: conn.type,
            esComposicion: conn.type === 'composition'
          });
          console.log(`         ← ${sourceElement.name} (muchos) apunta hacia ${element.name} (uno)`);
        }
        // 1:1 bidireccional
        else if (sourceMult === '1' && targetMult === '1') {
          // Ya manejado en el caso anterior
        }
      }
    });
  ...
  determinarForeignKey = (
    ...
    console.log(`         Source: ${sourceElement.name}`);
    console.log(`         Target: ${targetElement.name}`);
    
    // ✅ Normalizar multiplicidades
    const normalizeMultiplicity = (mult) => {
      if (!mult) return '1';
      mult = mult.toString().trim();
      // Convertir "1..*" → "*", "0..*" → "*", "n" → "*"
      if (mult === '1..*' || mult === '0..*' || mult === 'n' || mult === 'N' || mult === 'many') {
        return '*';
      }
      // Convertir "1..1" → "1"
      if (mult === '1..1') {
    ...
    const normalizedSourceMult = normalizeMultiplicity(sourceMult);
    const normalizedTargetMult = normalizeMultiplicity(targetMult);
    
    console.log(`         Multiplicidades normalizadas: ${normalizedSourceMult} → ${normalizedTargetMult}`);
    ...
    switch (tipoRelacion) {
      ...
      case "aggregation":
        // *:1 (Many-to-One)
        if (normalizedSourceMult === "*" && normalizedTargetMult === "1") {
          elementoConFK = sourceId;
          elementoReferenciado = targetId;
          tipoRelacionJPA = "@ManyToOne";
          console.log(`         → Detectado: Many-to-One (FK en ${sourceElement.name})`);
        } 
        // 1:* (One-to-Many)
        else if (normalizedSourceMult === "1" && normalizedTargetMult === "*") {
          elementoConFK = targetId;
          elementoReferenciado = sourceId;
          tipoRelacionJPA = "@ManyToOne";
          console.log(`         → Detectado: One-to-Many (FK en ${targetElement.name})`);
        } 
        // 1:1 (One-to-One)
        else if (normalizedSourceMult === "1" && normalizedTargetMult === "1") {
          elementoConFK = targetId;
          elementoReferenciado = sourceId;
          tipoRelacionJPA = "@OneToOne";
          console.log(`         → Detectado: One-to-One (FK en ${targetElement.name})`);
        }
        // *:* (Many-to-Many) - Requiere tabla intermedia
        else if (normalizedSourceMult === "*" && normalizedTargetMult === "*") {
          console.log(`         → Detectado: Many-to-Many (requiere tabla intermedia)`);
          // Para Many-to-Many, no agregamos FK directa
          return null;
        }
        break;
      ...
      case "composition":
        ...
        if (normalizedSourceMult === "*" && normalizedTargetMult === "1") {
          ...
          console.log(`         → Detectado: Composición Many-to-One`);
        } else if (normalizedSourceMult === "1" && normalizedTargetMult === "*") {
          elementoConFK = targetId;
          elementoReferenciado = sourceId;
          tipoRelacionJPA = "@ManyToOne";
          esClavePrimariaCompuesta = true;
          console.log(`         → Detectado: Composición One-to-Many`);
        }
        break;
        
      case "inheritance":
        console.log(`         → Herencia detectada (se maneja aparte)`);
        if (normalizedSourceMult === "*" && normalizedTargetMult === "1") {
      ...
      case "dependency":
      case "implementation":
        console.log(`         → ${tipoRelacion} (solo lógica, no genera FK)`);
        return null;
    }
    
    if (!elementoConFK || !elementoReferenciado) {
      console.log(`         ⚠️  No se pudo determinar elemento con FK`);
      return null;
    }
    ...
    console.log(`         ✅ FK determinada: ${fkName} (${fkType})`);
  ...
  generarAnotacionesRelacion = (relacionInfo) => {
    ...
    switch (relacionInfo.tipoRelacionJPA) {
      ...
      case "@OneToMany":
        // ✅ Remover sufijos ID/Id/id de forma case-insensitive para mappedBy
        anotaciones += `    ${
          relacionInfo.tipoRelacionJPA
        }(mappedBy = "${relacionInfo.fkName.replace(/ID$/i, "")}")\n`;
        if (relacionInfo.tipoRelacion === "composition") {
  ...
  generarCampoRelacion = (relacionInfo) => {
    // ✅ Remover sufijos ID/Id/id de forma case-insensitive
    const nombreCampo = relacionInfo.fkName.replace(/ID$/i, "");
    const tipoCampo = relacionInfo.referenciaA;
    let anotaciones = "";
    switch (relacionInfo.tipoRelacionJPA) {
      case "@ManyToOne":
    ...
    // ✅ SOLUCIÓN: Agregar @JsonProperty para mantener nombres exactos en JSON
    anotaciones += `    @JsonProperty("${nombreCampo}")\n`;
    return `    // Relación: ${relacionInfo.tipoRelacion} -> ${relacionInfo.referenciaA}
${anotaciones}    private ${tipoCampo} ${nombreCampo};
...
`;
  ...
  /**
   * Generar resto de la entidad CON relaciones bidireccionales
   */
  generarRestoEntidadCorregido = (
    className,
    element,
    relacionesElement,
    relacionesEntrantes,
...
`;
    ...
    if (element.attributes && element.attributes.length > 0) {
      const attributosBasicos = element.attributes.filter(
        (attr) => !attr.isForeignKey && !attr.isPrimaryKey
      );
      ...
      if (attributosBasicos.length > 0) {
        const params = attributosBasicos
          ...
          .join(", ");
        codigo += `    public ${className}(${params}) {
`;
        attributosBasicos.forEach((attr) => {
          codigo += `        this.${attr.name} = ${attr.name};
`;
        });
        codigo += `    }
...
`;
    ...
    codigo += `    // ===== GETTERS AND SETTERS =====

`;

    // Getter/Setter para PK
    if (pkField && pkType) {
      const capitalizedPk = this.capitalize(pkField);
      codigo += `    public ${pkType} get${capitalizedPk}() {
        return ${pkField};
    }
...
`;
    ...
    // Getters/Setters para atributos normales
    if (element.attributes && element.attributes.length > 0) {
      element.attributes.forEach((attr) => {
        if (!attr.isPrimaryKey && !attr.isForeignKey) {
          const capitalizedName = this.capitalize(attr.name);
          codigo += `    public ${attr.type} get${capitalizedName}() {
        return ${attr.name};
    }
...
`;
    ...
    // ✅ Getters/Setters para relaciones salientes (donde esta clase tiene FK)
    if (relacionesElement && relacionesElement.length > 0) {
      relacionesElement.forEach((relacion) => {
        if (relacion.tipoRelacion !== "inheritance") {
          // ✅ Remover sufijos ID/Id/id de forma case-insensitive
          const nombreCampo = relacion.fkName.replace(/ID$/i, '');
          const capitalizedRelacion = this.capitalize(nombreCampo);
          
          codigo += `    public ${relacion.referenciaA} get${capitalizedRelacion}() {
        return ${nombreCampo};
    }

    public void set${capitalizedRelacion}(${relacion.referenciaA} ${nombreCampo}) {
...
`;
    ...
    // ✅ Getters/Setters para relaciones entrantes (@OneToMany)
    if (relacionesEntrantes && relacionesEntrantes.length > 0) {
      relacionesEntrantes.forEach(rel => {
        if (rel.tipo === 'OneToMany') {
          const nombreCampo = rel.claseOrigen.toLowerCase() + 's';
          const capitalizado = this.capitalize(nombreCampo);
          
          codigo += `    public List<${rel.claseOrigen}> get${capitalizado}() {
        return ${nombreCampo};
    }

    public void set${capitalizado}(List<${rel.claseOrigen}> ${nombreCampo}) {
        this.${nombreCampo} = ${nombreCampo};
    }

    // Métodos helper para ${nombreCampo}
    public void add${rel.claseOrigen}(${rel.claseOrigen} item) {
        ${nombreCampo}.add(item);
        item.set${className}(this);
    }

    public void remove${rel.claseOrigen}(${rel.claseOrigen} item) {
        ${nombreCampo}.remove(item);
        item.set${className}(null);
    }

`;
        } else if (rel.tipo === 'OneToOne') {
          const nombreCampo = rel.claseOrigen.toLowerCase();
          const capitalizado = this.capitalize(nombreCampo);
          
          codigo += `    public ${rel.claseOrigen} get${capitalizado}() {
        return ${nombreCampo};
    }

    public void set${capitalizado}(${rel.claseOrigen} ${nombreCampo}) {
        this.${nombreCampo} = ${nombreCampo};
    }
...
`;
    ...
    codigo += `    // ===== UTILITY METHODS =====

    @Override
    public String toString() {
        return "${className}{" +
                "${pkField || 'id'}=" + ${pkField || 'id'}`;
